  ...
  <form method="POST" action="{{ url('vehicles') }}">
    ...
    <div class="row">
        ...
        <div class="form-group{{ $errors->has('cabnumber') ? ' has-error' : '' }}">
            ...
            <input id="cabnumber" type="text" class="form-control" name="cabnumber" value="{{ old('cabnumber') }}" required autofocus>
        ...
        <div class="form-group{{ $errors->has('driverid') ? ' has-error' : '' }}">
            ...
            <select id="driverid" class="form-control" name="driverid" autofocus>
              @foreach($drivers as $driver)
                <option value="{{ $driver->id }}" {{ old('driverid') == $driver->id ? 'selected' : '' }}>{{ $driver->getFullname() }}</option>
              @endforeach
            </select>
        ...
        <div class="form-group{{ $errors->has('operatorid') ? ' has-error' : '' }}">
            ...
            <select id="operatorid" class="form-control" name="operatorid" autofocus>
              @foreach($operators as $operator)
                <option value="{{ $operator->id }}" {{ old('operatorid') == $operator->id ? 'selected' : '' }}>{{ $operator->getFullname() }}</option>
              @endforeach
            </select>
    ...
    <div class="row">
        ...
        <div class="form-group{{ $errors->has('seats') ? ' has-error' : '' }}">
            ...
            <input id="seats" type="number" min="1" class="form-control" name="seats" value="{{ old('seats') }}" autofocus>
        ...
        <div class="form-group{{ $errors->has('available') ? ' has-error' : '' }}">
            ...
            <select id="available" class="form-control" name="available" autofocus>
                <option value="1" @if (old('available', 1) == 1) selected @endif>True</option>
                <option value="0" @if (old('available', 1) == 0) selected @endif>False</option>
            </select>
